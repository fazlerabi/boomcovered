...
export class AppComponent implements AfterViewInit {
    ...
    this.router.events.pipe(
      ...
      switch (event['url']) {
        ...
        case '/step8':
          ...
          break;
        case '/step9':
          (<HTMLElement>document.getElementsByClassName('red').item(0)).style.cssText =
            'background-color:#0644f4 !important;';
  ...
  applyHeaderColor() {
    ...
    let marginToNav, togglerColor = '255, 255, 255, 1';
    if (isHeaderCollapse || (this.router.url !== '/' && this.router.url !== '/index')) {
      (<HTMLElement>document.getElementsByClassName('navbar').item(0)).style.cssText =
        'box-shadow:none';
      this.navColor = 'black';
    ...
    (<HTMLElement>document.getElementsByClassName('navbar-toggler-icon')[0]).style.cssText =
      'background-image: url(\'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30 30">' +
      '<path stroke="rgba(' + togglerColor + ')" stroke-width="4" stroke-linecap="round" stroke-miterlimit="10" ' +
      'd="M4 7h22M4 15h22M4 23h22"/></svg>\') !important;';
